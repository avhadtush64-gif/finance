...
export default function Budgets() {
  ...
  return html`
    <div className="flex flex-col gap-6">
      ...
      ${!loading && budgets.length === 0 ? html`<${EmptyState} message="No budgets set yet." icon="🎯" />` : html`
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          ${budgets.map(b => html`<${BudgetCard} key=${b.id} budget=${b} onDelete=${handleDelete} />`)}
        </div>
      `}
